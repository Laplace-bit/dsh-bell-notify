...
在 **设置 → 插件 → 插件配置 → 铃声通知** 中管理启用状态、静音和总音量；它们持久化在当前 profile。每个事件的开关、自定义音源和文件名保存在浏览器本地（`localStorage` + IndexedDB），改完立即生效、刷新不丢。
...
**在哪里配置提示音？**
在 Web 设置的 **插件 → 插件配置 → 铃声通知** 中。这里可以设置启用状态、静音、音量、事件开关与自定义铃声，也能查看并更新 npm 安装的版本。
